...
//    }
    ...
    public static long getWorkerId(long dataCenterId, long maxWorkerId) {
        ...
        StringBuffer jvm_pid = new StringBuffer();

        jvm_pid.append(dataCenterId);
        String name = ManagementFactory.getRuntimeMXBean().getName();
        if (!name.isEmpty()) jvm_pid.append(name.split("@")[0]);

        return (jvm_pid.toString().hashCode() & 0xFFFF) % (maxWorkerId + 1); // MAC + PID 的 hashcode 获取16个低位
    }
     ...
     * @return 数据中心ID
     */
    public static long getDataCenterId(long maxDatCenterId) {
